...
conky.text = [[
${{voffset 30}}
${{goto 6}}${{color {cfg['colors']['accent']}}}${{font {cfg['fonts']['header']}}} Music ${{font}}

#current song
${{goto 20}}${{color {cfg['colors']['accent_dimmed']}}}${{execi {cfg['music']['update_interval']} bash -c '
...
# lyrics
${{execi 30 sh -c 'playerctl metadata --format "{{{{artist}}}} {{{{title}}}}" | python3 {BASE_DIR}/scripts/run.py | head -n 43'}}
# system info
${{goto 10}}-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
